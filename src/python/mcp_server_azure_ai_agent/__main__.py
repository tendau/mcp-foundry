...
class AzureAIAgentClient:
    ...
    def __init__(self):
        ...
        print(
            f"Azure AI Agent client initialized for AI Search connection: {self.search_connection_name}, Bing connection: {self.bing_connection_name}",
            file=sys.stderr,
        )

    def search_index(self, query, top=10):
        """
        ...
        try:
            ...
            search_connection = self.client.connections.get(
                connection_name=self.search_connection_name
            )
            if not search_connection:
                raise ValueError(
                    f"Connection '{self.search_connection_name}' not found"
                )

            # Create search tool
            search_tool = AzureAISearchTool(
                index_connection_id=search_connection.id, index_name=self.index_name
            )
    ...
    def web_search(self, query):
        ...
        try:
            # Get Bing connection
            bing_connection = self.client.connections.get(
...
@mcp.tool()
def search_index(query: str, top: int = 10) -> str:
    """Search your Azure AI Search index."""
...
def web_search(query: str) -> str:
    """Search the web with Bing Web Grounding."""
